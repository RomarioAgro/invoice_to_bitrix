...
- Номер задачи Битрикс необязателен: его можно очистить кнопкой «Не указывать», и тогда `UF_SEARCH_TASK` передаётся как пустая строка.
...
Запрос Битрикс содержит `TITLE`, поля `UF_INVOICE_*`, `UF_ARTICLES_DDS`, `UF_SEARCH_TASK` и один элемент `UF_INVOICE_FILES` с Base64 `DATA` и фактическим `EXT`. Для неуказанного номера задачи `UF_SEARCH_TASK` равен пустой строке. Любой HTTP 2xx считается успехом. HTTP-ошибка расходует попытку; timeout/ошибка соединения — нет.
