...
Telegram-бот для локального распознавания счетов PDF/XLSX и передачи проверенных реквизитов с исходным файлом в Битрикс.

## Специфика проекта

- Telegram API всегда вызывается через заданный SOCKS5 Xray.
- Битрикс всегда вызывается отдельным HTTP-клиентом с `trust_env=False`.
- Реальную отправку тестового счёта в production выполнять только после подтверждения конкретного файла и задачи пользователем.

## Memory Bank

В проекте используется `memory-bank/`. Перед каждой существенной задачей прочитай шесть основных файлов. После изменений обнови затронутые документы, обязательно `activeContext.md` и `progress.md`.

## Управление задачами

Проект использует mdtask. Перед разработкой выполни `mdtask list` и `mdtask view <ID>`. Выполняй одну задачу за цикл.

<!-- ontoindex:start -->
# OntoIndex — Code Intelligence

This project is indexed by OntoIndex as **invoice_to_bitrix** (818 symbols, 1246 relationships, 15 execution flows). Use the OntoIndex MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `ontoindex://repo/invoice_to_bitrix/context` | Codebase overview, check index freshness |
| `ontoindex://repo/invoice_to_bitrix/clusters` | All functional areas |
| `ontoindex://repo/invoice_to_bitrix/processes` | All execution flows |
| `ontoindex://repo/invoice_to_bitrix/process/{name}` | Step-by-step execution trace |
