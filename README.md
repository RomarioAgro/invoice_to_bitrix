...
- При ручном вводе дата счёта принимается как `ДД.ММ.ГГГГ`, срок оплаты — как `ДД.ММ.ГГГГ` или `ДД,ММ,ГГГГ`; перед отправкой запятые в сроке оплаты заменяются точками.
- ИНН распознаётся как отдельное поле (`ИНН`, `ИНН:`, `ИНН №`) и в общей банковской подписи `ИНН/КПП`, включая пробелы вокруг `/`.
- Непустая подпись Telegram к PDF/XLSX после удаления краевых пробелов становится описанием счёта; внутренние переносы сохраняются. Без подписи бот запрашивает описание отдельно, из содержимого файла оно не извлекается.
...
Compose использует host network, чтобы контейнер обращался к SOCKS5 по `127.0.0.1`; приложение не слушает входящие порты. Контейнер запускается непривилегированным пользователем и монтирует постоянные `data`/`logs`. Для резервной копии остановите контейнер и архивируйте эти каталоги; для очистки удаляйте только их содержимое при остановленном контейнере.

## Xray на VPS

Xray работает как systemd-служба и по явному требованию владельца слушает все IPv4-интерфейсы на `0.0.0.0:10808`. Бот продолжает обращаться к нему через `127.0.0.1:10808`. Проверки:
