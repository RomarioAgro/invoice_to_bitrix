...
- Номер задачи Битрикс необязателен: его можно очистить кнопкой «Не указывать», и тогда `UF_SEARCH_TASK` отсутствует в JSON.
...
Запрос Битрикс содержит `TITLE`, поля `UF_INVOICE_*`, `UF_ARTICLES_DDS` и один элемент `UF_INVOICE_FILES` с Base64 `DATA` и фактическим `EXT`. `UF_SEARCH_TASK` добавляется только при указанном положительном номере задачи. Любой HTTP 2xx считается успехом. HTTP-ошибка расходует попытку; timeout/ошибка соединения — нет.
